...
class NewsModel extends Model {
    ...
    protected $table = 'aldiar_news_tb';
...
//        $mNews->description = Input::get('description');
        ...
        //thumbnail
        $thumbnail = FileHelper::storeImage('img','uploads/news',320,180);
        if(!empty($thumbnail)){
            $mNews->thumbnail = $thumbnail;
        }else{
            $mNews->thumbnail = NewsModel::DEFAULT_MEDIA_COVER;
        }

        //image
        $img = FileHelper::storeImage('img','uploads/news',865,400);
        if(!empty($img)){
            $mNews->img = $img;
        }else{
            $mNews->img = NewsModel::DEFAULT_MEDIA_COVER;
        }
